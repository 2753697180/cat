    ...
    std::vector<std::pair<Real, Real>> & outgoing_vals)
{
  ...
  const unsigned int num_z_segments = 11;
   const Real z_min = 0.0;
   const Real z_max = 0.55;
   const Real dz = (z_max - z_min) / num_z_segments;
   std::vector<Real> z_segment_sums(num_z_segments, 0.0);
   std::vector<unsigned int> z_segment_counts(num_z_segments, 0);
  for (const auto i_from : make_range(_num_sources))
  {
    ...
    if (!checkRestrictionsForSource(pt, mesh_div, i_from))
      continue;
    Real z = _local_points.z ; // 获取 z 坐标
    int num=z%dz;
    z_segment_sums[num]+=_local_value   
