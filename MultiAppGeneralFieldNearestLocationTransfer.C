...
{
  dof_id_type i_pt = 0;
      std::vector<std::map<unsigned int, std::vector<Real>>> all_z_bins(_num_sources);//三层数组，第一层：app索引；第二层：段索引;第三层：段对应z坐标
      // 划分长度
      Real total_length = 0.55;
      unsigned int num_bins = 10;  //  段
      Real bin_width = total_length / num_bins; 
  ...
  { // Examine all restrictions for the point. This source (KDTree+values) could be ruled out
      // 每个段的宽度是 0.04m
        for (const auto i_from : make_range(_num_sources))
      {
        for (std::size_t i = 0; i < _local_points[i_from].size(); ++i)
        {
          const Point & pt = _local_points[i_from][i];
          Real z = pt(2); // 取 z 坐标
         unsigned int bin_index = static_cast<unsigned int>((z) / bin_width);
          // 计算 bin_index，右闭区间：[a, b]
          if (bin_index >= num_bins)
               { bin_index = num_bins - 1;}
          // 如果 z 是 bin_width 的整数倍，放入当前区间
          // 将值放入对应的 z 区间
          all_z_bins[i_from][bin_index].push_back(_local_values[i_from][i]);
        }
      }        
  }
  ...
  {
        Real z = pt(2);
        unsigned int tgt_bin = static_cast<unsigned int>(z / bin_width);
        if (tgt_bin >= num_bins)
            tgt_bin = num_bins - 1;
        if (tgt_bin > 0 &&
            std::fabs(z - tgt_bin * bin_width) < std::numeric_limits<Real>::epsilon())
            tgt_bin--;
  ...
  for (const auto i_from : make_range(_num_sources))
  {
    if (!checkRestrictionsForSource(pt, mesh_div, i_from))
      continue;
    const auto & bin_map = all_z_bins[i_from];
    auto it = bin_map.find(tgt_bin);
    if (it != bin_map.end() && !it->second.empty())
    {
      point_found = true;
      const auto & values = it->second;
      Real sum = std::accumulate(values.begin(), values.end(), 0.0);
      Real avg = sum / values.size();
      outgoing_vals[i_pt] = {avg, 0.0};
      break;
    }
  }
